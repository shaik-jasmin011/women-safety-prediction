...
let audioCtx = null;
let alarmNodes = [];   // keep refs to stop them
...
function playAlarmLoop() {
  if (!audioCtx) return;
  // Two alternating tones — classic emergency siren pattern
  const tones = [880, 660];
  let time = audioCtx.currentTime;

  for (let i = 0; i < 20; i++) {          // 20 pairs = ~10 seconds, then restarts
    tones.forEach(freq => {
  ...
  // Restart loop after 10 seconds if alarm still active
  alarmTimeout = setTimeout(() => {
    if (audioCtx) playAlarmLoop();
  }, 10000);
}
...
function triggerSOS() {
  ...
  addLog('🔊 Alarm sound activated', 'danger');

  const locEl = document.getElementById('sosLocation');
  locEl.textContent = '📍 Fetching your location…';

  // Always try to get fresh location at SOS time
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        locEl.innerHTML = `📍 <a href="${link}" target="_blank" style="color:#c084fc;word-break:break-all">
          ${currentLat.toFixed(5)}, ${currentLng.toFixed(5)}</a>
          <br><small style="color:#64748b">Accuracy: ±${Math.round(pos.coords.accuracy)}m</small>`;
        addLog(`📍 Location: ${currentLat.toFixed(4)}, ${currentLng.toFixed(4)}`, 'success');
        // Open WhatsApp with location if contacts exist
        if (contacts.length > 0) {
          const msg = encodeURIComponent(
            `🆘 EMERGENCY! I need help.\n📍 My location: https://maps.google.com/?q=${currentLat},${currentLng}\n⏰ ${new Date().toLocaleTimeString()}`
          );
          addLog('💬 WhatsApp message ready — tap below to send', 'success');
          // Add WhatsApp button inside overlay
          const existing = document.getElementById('sosWhatsappBtn');
          if (!existing) {
            const waBtn = document.createElement('a');
            waBtn.id = 'sosWhatsappBtn';
            waBtn.href = `https://wa.me/?text=${msg}`;
            waBtn.target = '_blank';
            waBtn.className = 'btn btn--secondary';
            waBtn.style.marginTop = '0.8rem';
            waBtn.textContent = '💬 Send Location via WhatsApp';
            document.getElementById('dismissSosBtn').insertAdjacentElement('beforebegin', waBtn);
          }
        }
      },
      err => {
        locEl.textContent = '📍 Location access denied — please allow location';
        addLog('⚠️ Location error: ' + err.message, 'danger');
      },
      { enableHighAccuracy: true, timeout: 8000 }
    );
  } else {
    locEl.textContent = '📍 Geolocation not supported';
    addLog('⚠️ Geolocation not supported', 'danger');
  }

  contacts.forEach(c => addLog(`📱 Alert queued for ${c.name} (${c.phone})`, 'success'));
  showToast('🚨 SOS Activated! Fetching location…', 'error');
}
...
function dismissSOS() {
  ...
  // Remove WhatsApp button if it was added
  const waBtn = document.getElementById('sosWhatsappBtn');
  if (waBtn) waBtn.remove();
  addLog('✅ SOS dismissed — alarm stopped', 'info');
  showToast('SOS cancelled. Alarm stopped.', 'info');
}
